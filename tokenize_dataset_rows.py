...
def preprocess(tokenizer, config, example, max_seq_length, version):
    ...
    if version == 'v2':
        ...
        a_ids = tokenizer.encode(text=prompt,
                                 add_special_tokens=True,
                                 truncation=True,
                                 max_length=max_seq_length)
        b_ids = tokenizer.encode(text=target,
                                 add_special_tokens=False,
                                 truncation=True,
                                 max_length=max_seq_length)
...
def read_jsonl(path,
    ...
    config = transformers.AutoConfig.from_pretrained(chatglm_path,
...
def main():
    ...
    parser.add_argument("--jsonl_path",
                        type=str,
                        default="./data/alpaca_data.jsonl")
